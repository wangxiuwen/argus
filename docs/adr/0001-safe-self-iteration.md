# ADR-0001: Self-iteration produces candidates before changing Mira

Status: accepted

## Context

Mira should improve Creations, remember Preferences, propose source changes, and train adapters. Source edits and weight training can consume substantial resources or damage a working installation if performed silently.

## Decision

Automatic Iterations may refine prompts, retry failed Quality Gates, and apply explicit Preferences. Code and model Iterations must first produce a Candidate in isolated storage. Applying a code Candidate or starting a Training Run requires a separate explicit Approval. Mira never installs a Candidate over the running application automatically.
...
- Closing Mira does not lose Iteration state.
