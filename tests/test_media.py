...
class UnifiedConversationTests(unittest.TestCase):
    def test_agent_chooses_media_tools_from_one_chat_composer(self):
        ...
        self.assertIn('status:"planning", requestId', html)
        self.assertIn("request_id: record.requestId", html)
        self.assertIn('m.status === "planning"', html)
        self.assertIn("recoverUnlinkedTasks()", html)
