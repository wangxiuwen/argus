...
- **Durable batch generation** — requests such as “create 100 songs” become persistent SQLite-backed batches with per-item prompts, progress, pause, resume and cancel controls. Closing the window or restarting Fermi does not discard the queue.
...
- **Safe source iteration** — Fermi can ask a coding agent to improve an isolated checkout, run its tests and present the result as a Candidate. It never edits or replaces the running application silently.
...
- **Local video generation** — MiniMax H3 produces video with synchronized stereo audio. Fermi handles the source download, visible progress, 4-bit conversion and inline playback.
- **Desktop chat app** — **Open Fermi** in the menu bar opens a native window (AppKit + WebKit) with past conversations, images by ＋ / drag & drop / ⌘V, Agent replies and live generation task cards. Enter inserts a line break; ⌘↵ or Ctrl+Enter sends. No Electron.
...
- **Model switching** — pick bf16 / 8bit / 4bit from the tray submenu or the picker next to the message box; Fermi rewrites the config, restarts the server, and downloads the weights if you don't have them yet. Already-downloaded variants are marked ✓.
- **CLI** — `fermi ask "what's this?" photo.png` for one-shot questions, `fermi chat` for an interactive session (drop image paths straight into your message), `fermi ui` for the chat page in a browser, plus `start|stop|restart|status|log|use|download|model|config`.
...
- **Resilient downloads** — Fermi defaults to resumable HTTPS downloads instead of Xet/CAS chunk reconstruction, which is less reliable behind some mirrors and proxies. Set `HF_HUB_DISABLE_XET=0` when starting Fermi to opt back into Xet.
- **Automatic cache cleanup** — before each start, Fermi removes only superseded partials for the selected model while retaining the newest partial for resume. Other models in the shared Hugging Face cache are never touched.
...
can reuse the same snapshots and interrupted downloads can resume. Fermi keeps
existing tasks in the legacy-compatible `~/Library/Application Support/Mira` location; locally converted
...
Click the menu bar atom → **Start Server**. The first run downloads the model; later runs load it in ~1–2 minutes. When the status says ready, the API is available:
...
tool calls. Fermi supplies `generate_images`, `generate_music`,
...
Fermi first reports the estimated output size and asks for confirmation. After
...
changes and model training cannot: Fermi first creates a reviewable Candidate,
...
```bash
fermi start       # start (auto-downloads model on first run)
fermi stop
fermi restart
fermi status      # ready / loading / not running
fermi log         # tail the server log

fermi ask "what is in this image?" shot.png   # one-shot, images optional
fermi ask --think "17*23?"                    # let it reason first
fermi chat        # interactive; /think toggles reasoning, /new resets
fermi ui          # open the chat interface in a browser

fermi use mlx-community/Qwen3.8-27B-4bit      # switch model and restart
fermi download    # pre-download the configured model
fermi config      # show effective settings
```

### Coding tools

Fermi can launch coding agents against the local model. The compatibility
bridge keeps each client from replacing the model already loaded in memory.

```bash
fermi launch                         # list supported tools and install status
fermi launch codex                   # Codex CLI
fermi launch aider --message "help" # aider (extra arguments pass through)
fermi launch opencode                # OpenCode
fermi launch claude                  # Claude Code via Anthropic Messages API
fermi launch shell                   # shell with local API variables set
fermi bridge status                  # inspect the bridge separately
```
...
```bash
ARGUS_CLAUDE_FULL=1 fermi launch claude
```
...
your normal OpenCode plugins/MCPs while still forcing inference through Fermi:

```bash
ARGUS_OPENCODE_FULL=1 fermi launch opencode
```
...
`MAX_TOKENS` controls both Fermi clients and the server-side generation cap.
...
Chat model weights live in `~/.cache/huggingface/hub`; media weights use Fermi's
