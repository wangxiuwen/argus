#!/usr/bin/env python3
"""Fermi CLI chat client — talks to the local mlx-vlm OpenAI-compatible server.
...
def get_model():
    try:
        ...
        sys.exit(f"server not reachable at {API} — start it with: fermi start")
...
def stream_chat(messages, model, thinking=False):
    ...
    try:
        ...
        sys.exit(f"server not reachable at {API}: {reason} — start it with: fermi start")
...
def cmd_ask(argv):
    ...
    for flag in ("--think", "--thinking"):
        if flag in argv:
            ...
            thinking = True
    if not argv:
        sys.exit('usage: fermi ask [--think] "prompt" [image.png ...]')
...
def cmd_chat():
    model = get_model()
    thinking = False
    print(f"Fermi chat — model: {model}")
